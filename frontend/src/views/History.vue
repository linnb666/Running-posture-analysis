<template>
  <div class="history-page">
        ...
        >
          ...
          <div class="record-actions">
            <el-button
              ...
              type="primary"
              link
              @click.stop="openRenameDialog(record)"
            >
              重命名
            </el-button>
            <el-button
    ...
    <el-dialog v-model="renameDialogVisible" title="重命名测试记录" width="420px">
      <el-input
        v-model="renameName"
        maxlength="120"
        show-word-limit
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="renameLoading" @click="submitRename">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
function openRenameDialog(record) {
  ...
  renameName.value = record.video_filename || ''
  renameDialogVisible.value = true
}

async function submitRename() {
  if (!renameTargetId.value || renameLoading.value) return
  ...
  if (name.length > 120) {
    ElMessage.warning('名称长度不能超过120')
    return
  }

  renameLoading.value = true
  try {
    const resp = await historyApi.renameHistory(renameTargetId.value, name)
    const newName = resp?.record?.video_filename || name
    const item = records.value.find((x) => x.id === renameTargetId.value)
    if (item) item.video_filename = newName
