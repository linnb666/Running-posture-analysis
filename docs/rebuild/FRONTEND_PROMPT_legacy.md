...
> 说明：本文档为历史保留文档，主要用于回滚参考和过程追溯；若与当前代码冲突，应以代码和 `docs/engine/` 下主文档为准。
...
```python
{
    ...
    'vertical_motion': {
        'amplitude_normalized': float,  # 垂直振幅 (躯干长度 %)
        'amplitude_rating': {
...
**说明**：
- 注册/登录是 MVP 必做功能（不再可选）
- 登录成功返回 `access_token` 与 `refresh_token`
- 业务接口默认要求 `Authorization: Bearer <access_token>` 头
...
Body:
  file: <视频文件>
  view_angle: 'side' | 'front'     (必填)
  enable_3d: true | false          (可选，默认 true)
```
...
```
GET /api/history?page=1&limit=20&view_angle=side
```
...
```json
{
  ...
  "records": [
    {
      "id": 123,
      "video_filename": "running_001.mp4",
      "view_angle": "side",
      "total_score": 78.4,
      "rating": "good",
...
socket.on('analysis_progress', (data) => {
  // data: { task_id: 'xxx', progress: 45, stage: '姿态估计中...' }
});
